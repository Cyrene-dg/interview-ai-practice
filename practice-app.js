(() => {
  ...
  function renderPractice() {
    ...
    function drawQuestion() {
      ...
      app.innerHTML = `<div class="question-head"><div class="crumb"><a href="${document.querySelector('#backToCatalog').href}">题库目录</a><i>/</i><span>${escapeHtml(question.source)}</span><i>/</i><span>${escapeHtml(question.category)}</span></div><div class="question-badges"><span class="tag">第 ${escapeHtml(question.number)} 题</span><span class="plain-tag">${escapeHtml(question.type)}</span></div>${title}<h1>${lineBreaks(displayContent)}</h1>${formattedCode?.html || ''}</div>${options}<div id="answerArea">${revealed ? showAnswer(record) : ''}</div><div class="practice-actions"><button id="previousQuestion" class="secondary" ${previous ? '' : 'disabled'}>上一题</button><div>${mainAction}</div><button id="nextQuestion" class="secondary" ${next ? '' : 'disabled'}>下一题</button></div><div id="noteArea"></div>`;
