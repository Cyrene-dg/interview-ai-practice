  ...
  function renderPractice() {
    ...
    function attachNoteEditor() {
      ...
      noteArea.querySelector('#exportSingleNote').onclick = async () => {
        ...
        const markdown = [`# ${question.source} / ${question.category} · 第 ${question.number} 题`, '', `## ${question.title || question.content}`, '', finalNote.summary ? `> 复习结论：${finalNote.summary}` : '', finalNote.personal ? '## 我的理解\n\n' + finalNote.personal : '', finalNote.ai ? '## AI 辅助解析\n\n' + finalNote.ai : ''].filter(Boolean).join('\n\n') + '\n';
    ...
    function drawQuestion() {
      ...
      app.innerHTML = `<div class="question-head"><div class="crumb"><a href="${document.querySelector('#backToCatalog').href}">题库目录</a><i>/</i><span>${escapeHtml(question.source)}</span><i>/</i><span>${escapeHtml(question.category)}</span></div><div class="question-badges"><span class="tag">第 ${escapeHtml(question.number)} 题</span><span class="plain-tag">${escapeHtml(question.type)}</span></div>${title}<h1>${lineBreaks(question.content)}</h1></div>${options}<div id="answerArea">${revealed ? showAnswer(record) : ''}</div><div id="noteArea"></div><div class="practice-actions"><button id="previousQuestion" class="secondary" ${previous ? '' : 'disabled'}>上一题</button><div>${mainAction}</div><button id="nextQuestion" class="secondary" ${next ? '' : 'disabled'}>下一题</button></div>`;
