(() => {
  ...
  function renderCatalog() {
    ...
    const categoryButtons = hasGroups ? course.groups.map(item => `<section class="directory-group"><button class="directory-parent ${item.name === group ? 'active' : ''}" data-group="${escapeHtml(item.name)}" type="button"><span>${escapeHtml(item.name)}</span><em>${item.count}</em><b aria-hidden="true">⌄</b></button><div class="directory-children" data-group-children="${escapeHtml(item.name)}" ${item.name === group ? '' : 'hidden'}>${item.topics.map(topic => `<button class="side-button category directory-leaf ${item.name === group && topic.name === category ? 'active' : ''}" data-leaf-group="${escapeHtml(item.name)}" data-category="${escapeHtml(topic.name)}" type="button"><span>${escapeHtml(topic.name)}</span><em>${topic.count}</em></button>`).join('')}</div></section>`).join('') : course.categories.map(item => `<button class="side-button category ${item.name === category ? 'active' : ''}" data-category="${escapeHtml(item.name)}" type="button"><span>${escapeHtml(item.name)}</span><em>${item.count}</em></button>`).join('');
