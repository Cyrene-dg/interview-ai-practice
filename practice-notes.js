(() => {
  ...
  function draw() {
    ...
    const shown = allNotes.filter(note => !key || `${note.source} ${note.category} ${note.number} ${note.title} ${note.summary} ${note.personal} ${note.ai}`.toLowerCase().includes(key));
  ...
  document.querySelector('#exportReadable').onclick = async () => {
    ...
    value.forEach(note => text.push(`## ${note.source} / ${note.category} · 第 ${note.number} 题`, '', `**${note.title}**`, '', note.summary ? `> 复习结论：${note.summary}` : '', note.personal ? '### 我的理解\n\n' + note.personal : '', note.ai ? '### AI 辅助解析\n\n' + note.ai : '', ''));
